...
export default function HowWeExportPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <Header />
      <main className="flex-1 bg-brand-gold-light/10 pt-24">
        {/* Hero */}
            ...
            <div className="grid lg:grid-cols-2 gap-16">
              ...
              <div className="space-y-10">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  ...
                  <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                    <h4 className="font-bold text-brand-gold mb-2">Legal Compliance</h4>
                    <p className="text-sm text-brand-gold-light/70">CE, FDA, and SASO compliant barcoding and labelling per destination requirements.</p>
                  </div>
                  <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                    <h4 className="font-bold text-brand-gold mb-2">Palletization</h4>
                    <p className="text-sm text-brand-gold-light/70">ISPM-15 heat-treated wooden or plastic pallets available for FCL shipments.</p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-6">
                  <div className="bg-brand-gold/10 p-8 rounded-2xl border border-brand-gold/30">
                    ...
                    <div className="flex flex-wrap gap-3 mb-8">
                      {['T/T (Wire)', 'LC at Sight', 'DP (Doc Against Pay)', 'DA (Doc Against Accept)'].map(m => (
                        <Badge key={m} variant="outline" className="bg-white/10 text-white border-white/20 px-4 py-1">{m}</Badge>
                      ))}
                    </div>
                    <h4 className="text-brand-gold font-bold uppercase tracking-widest text-xs mb-4">Trading Currencies</h4>
                    <div className="flex flex-wrap gap-3">
                      {['USD', 'EUR', 'GBP', 'AED'].map(c => (
                        <span key={c} className="font-bold text-2xl">{c}</span>
                      ))}
                    </div>
                  </div>
                  <div className="flex flex-col gap-4">
                    <Button size="lg" className="bg-brand-gold text-brand-navy hover:bg-brand-gold/90 h-14 text-lg font-bold">
                      Get a Pro Forma Invoice <ArrowRight className="ml-2 w-5 h-5" />
                    </Button>
                    <p className="text-center text-xs text-brand-gold-light/50">Credit facilities available for repeat buyers with a valid trade history.</p>
                  </div>
                </div>
              </div>
            </div>
