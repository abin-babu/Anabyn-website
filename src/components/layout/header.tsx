...
import { Menu, X, ChevronDown, Package, ShieldCheck, Truck, Droplets, Bed, Settings, FileDown } from 'lucide-react';
    ...
    >
      <div className="container px-4 mx-auto flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-2 rtl:space-x-reverse">
          ...
          {navLinks.map((link) => (
            <Link key={link.label} href={link.href as any} className="px-4 py-2 transition-colors">
              {link.label}
            </Link>
          ))}
          
          <div className="pl-6 border-l border-white/10 ml-4 flex items-center gap-4">
            <Link href="/catalogue" className="text-white/60 hover:text-brand-gold transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
      ...
      <div className={cn(
        ...
        <div className="relative h-full flex flex-col p-6">
          ...
          <nav className="flex flex-col gap-8 text-center">
            <Link href="/products" className="text-3xl font-playfair font-bold text-white" onClick={() => setIsMobileMenuOpen(false)}>{t('products')}</Link>
            {navLinks.map((link) => (
              <Link key={link.label} href={link.href as any} className="text-3xl font-playfair font-bold text-white" onClick={() => setIsMobileMenuOpen(false)}>{link.label}</Link>
            ))}
            <Link href="/catalogue" className="text-2xl font-playfair font-bold text-brand-gold" onClick={() => setIsMobileMenuOpen(false)}>Product Catalogue</Link>
          </nav>

          <div className="mt-auto pb-12">
            <Link href="/request-quote" className="w-full btn-quote-nav py-8 text-lg" onClick={() => setIsMobileMenuOpen(false)}>{t('requestQuote')}</Link>
          </div>
        </div>
      </div>
    </header>
